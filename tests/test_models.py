...
class TestProductModel(unittest.TestCase):
    ...
    def test_update_a_product(self):
        ...
        product.create()
        self.assertIsNotNone(product.id)
        product.description = "peaceall"
        original_id = product.id
        product.update()
        self.assertEqual(product.id, original_id)
        self.assertEqual(product.description, "peaceall")
        products = Product.all()
        self.assertEqual(len(products), 1)
        self.assertEqual(products[0].id, original_id)
        self.assertEqual(products[0].description, "peaceall")

    def test_update_a_product_with_invalid_id(self):
        """It should raise an exception on update with invalid id"""
        product = ProductFactory()
        product.id = None
    ...
    def test_product_serialization(self):
        ...
        self.assertTrue(isinstance(product_dict, dict))
        ...
        self.assertEqual(product.price, Decimal(product_dict["price"]))
    ...
    def test_deserialize_product(self):
        ...
        self.assertEqual(str(product), "<Product Bread id=[None]>")
        self.assertTrue(product is not None)
        self.assertEqual(product.id, None)
        self.assertEqual(product.name, "Bread")
        self.assertEqual(product.description, "testing")
        self.assertEqual(product.available, True)
        self.assertEqual(product.price, 120.6)
        self.assertEqual(product.category, Category.FOOD)

    def test_deserialize_product_data_error(self):
        """It should raise an exception on data error"""
        product = Product()
    ...
    def test_deserialize_product_attribute_missing_error(self):
